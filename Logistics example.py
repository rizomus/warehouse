...
cols = ['Arrival time', 'Product list']   # Подлежащие погрузке
...
for sec in range(0, 60*60*2, SECONDS_IN_FRAME):
    ...
    if truck_n < len(DF_TRUCK) and time >= DF_TRUCK['Arrival time'][truck_n]:       # Если подошло время прибытия очередной фуры
        Truck('Truck', truck_vertex, speed=0, graph=GRAPH, icon=truck_icon)                                       # 
        truck.n = truck_n
        trucks.append(truck)
        print('New truck arrived to ' + truck.vertex)
        products = DF_TRUCK['Product list'][truck_n].copy()
        products['Target truck'] = [truck for _ in range(min(10, len(products)))]
        prod_list = pd.concat([prod_list, products], axis=0)
        truck_n += 1
    ...
    for agent in AGENTS:
        if agent.busy() or agent.tasks:
            ...
            if len(prod_list) != 0:
                pallet_vertex = prod_list['Vertex'].iloc[0]
                target_truck = prod_list['Target truck'].iloc[0]
                tier = prod_list['Tier'].iloc[0]
                letter, num = pallet_vertex.split('-')
                num = int(num)
                rack = DF_RACK[letter][num]
                agent.add_task('go', pallet_vertex,)
                agent.add_task('do_something', 60, 'taking pallet from rack')
                agent.add_task('take_pallet', rack, None, tier)
                agent.add_task('go', target_truck.vertex,)
                agent.add_task('do_something', 60, 'putting pallet to truck')
                agent.add_task('give_pallet', target_truck,)
                index_0 = prod_list.index[0]
                prod_list = prod_list.drop(index_0, axis=0)
            else:                                               # if no work required
                reward -= 0.05                                     # non-productive time

    for truck in trucks:
        print(f'Truck {truck.n} loaded: {len(truck.pallets)/10*100:.0f}%, prod_list: {len(prod_list)}, reward: {reward:.0f}')
        if len(truck.pallets) == 10:
            reward += 200
            trucks.remove(truck)        # Full loaded truck
        else:
            reward -= 0.1

    text = f'Reward: {reward:.1f}'
    img = render([*trucks, *AGENTS], text=text, return_img=True)
    cv2.imwrite(f'/content/frames/img_{sec/10000:.4f}.jpg', img)
